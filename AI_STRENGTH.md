...
>
> 2026-06 更新：配对评测基线 vs 3×L5 = **35.4%**(480 局, §6)；重训/调参五条路全部
> 实测无增益(§7)，现役配置即角色选择架构下的最优。宗师实际机制 = **policy 先验 ×
> 完整 rollout**(value 头未参与决策, §1 修正)。
...
与 §4/§5 合并后的全景：AlphaZero 自对弈、强价值、联赛多样性、BR 重训、超参扫描**五条路
全部排除**。在"L4/L5/L6 共用子决策启发式"的架构下，宗师对 L5 的 ~35% 已贴近该架构天花板；
要再往上，必须**打破共用启发式**(给 L6 独立的、可调优的子决策策略——如 CMA-ES 调 L6 私有
参数、或终局精确求解器)，而非继续优化角色选择。

附带产出：`sim_nn.js` 载入后释放嵌套权重(每进程省 ~100MB)；`window._alphaC`/
`window._alphaEndBoost` 调参钩子；`tools/fallback_probe.js`(验证评测局搜索真实性)。
