...
| 臂 | 配置 | 胜率 | 配对差 vs A | z |
|---|---|---|---|---|
| A | 现役（完整 rollout，alphaIters 400） | （测量中） | — | — |
| B | vnet 叶评估，等墙钟 alphaIters 1645（×4.11） | （测量中） | （测量中） | （测量中） |
| C | vnet 叶评估，同迭代 400 | （测量中） | （测量中） | （测量中） |

B 与 A 每决策墙钟相同（1645×272µs ≈ 400×1118µs ≈ 447 ms）；C 隔离"估值质量"（同迭代）与 B 的"模拟更多"。

**判定**：（测量中）。部署规则不变：z>1.96 才切默认；未过线则保留 opt-in 旋钮与全部工具链。
