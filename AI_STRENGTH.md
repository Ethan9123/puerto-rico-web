...
>
> **2026-09（Phase 1–5，见 §13–§13.8）**：一条净收益、四条负结果、一条阻塞。
...
> 唯一未测的正统路线是 AlphaGo 式 **λ 混合**叶评估（能力已建好 `sim.js opts.rolloutFrac`）。
...
一个换建造子决策），却输在同一个数量级上。合起来给出一致的解释：

> **价值网擅长"预测这局大概会赢多少"，不擅长"分辨两个相似局面谁更好"。**
> 它的 MSE 优势来自把**整体水平**估得准；而选建筑要求分辨的是**彼此非常接近**的候选局面，
> 此时系统性偏差不抵消，反而盖过了信号。手工 `evalBuildingValue` 里编码的领域知识
> （工厂/港口/公会大厅的具体协同、产业链完成度、卡位）恰恰是针对这种细粒度比较调出来的。
