...
## 16. 算力缩放探针：评测口径与部署口径差 55 倍，且 L6 的搜索**未饱和**
...
- **部署（浏览器）**：`game.js:1856` 默认预算 alphaIters 40000 / alphaMs 8000，
  `game.js:4139` K = min(4, cores−1) 个 worker 根并行；§12 实测 3 worker 合并 **21,824 迭代/决策**。
  约束是 8 s 的墙钟，不是迭代数。
- **评测（本文档全部强度结论）**：`tools/eval_paired_worker.js` 固定 **alphaIters=400**，iter-bounded。

差 **~55 倍**。§2/§3/§7/§13 里的每一个 pp 数都是在低 55 倍的算力下测得的，
而此前从没有人测过：**L6 是否仍随迭代数变强？** 这决定"让每次迭代更便宜"是不是一条真杠杆，
...
**含义**：固定 8 s 预算下迭代数 ×1.137。按探针斜率（+3.5pp/翻倍）折算约 **+0.8pp**——
真实、免费、**测不出来**。不宣称棋力提升。要吃到可测的量（一个翻倍），需要更深的改动
（避免每迭代整状态 clone、更便宜的 rollout 策略），留作后续。
